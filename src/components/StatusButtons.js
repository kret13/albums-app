    ...
    <React.Fragment>
      {listItem ? (
        Boolean(listItem.finishDate) ? (
          <TooltipButton
            label="Mark as 'to listen'"
            highlight={colors.yellowDarken10}
      ...
      {listItem ? (
        <TooltipButton
          label="Remove from 'to listen' list"
        ...
        <TooltipButton
          label="Add to 'to listen' list"
          highlight={colors.yellowDarken10}
          onClick={() => handleAddClick({albumId: album.id})}
          icon={<FaPlusCircle />}
        />
      )}
    </React.Fragment>
