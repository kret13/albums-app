...
function install() {
  ...
  function DevTools() {
    ...
    const [persist, setPersist] = useLocalStorageState(
      '__albumsapp_devtools_persist__',
      false,
    )
    const [tabIndex, setTabIndex] = useLocalStorageState(
      '__albumsapp_devtools_tab_index__',
      0,
    )
      ...
      <div
        css={{
          ...
          'input, select': {
            ...
            ':focus': {
              outlineColor: colors.yellowDarken10,
              borderColor: colors.yellowDarken10,
              outline: '1px',
            },
          },
          'button:not([data-reach-tab])': {
            borderRadius: 5,
            background: colors.yellowDarken10,
            ':hover': {
              background: colors.yellowDarken10,
            },
            border: 0,
            ...
            <button
              css={{
                ...
                '::before': {
                  ...
                  left: 0,
                  top: 0,
                  background: persist ? colors.yellowDarken10 : 'transparent',
                },
              }}
              onClick={toggleShow}
            >
...
function RequestMinTime() {
  const [minTime, setMinTime] = useLocalStorageState(
    '__albumsapp_min_request_time__',
    400,
  )
...
function RequestVarTime() {
  const [varTime, setVarTime] = useLocalStorageState(
    '__albumsapp_variable_request_time__',
    400,
  )
...
function RequestFailUI() {
  const [failConfig, setFailConfig] = useLocalStorageState(
    '__albumsapp_request_fail_config__',
    [],
  )
